...
export async function getCorsiByFacoltaNome(collectionName, facoltaNome) {
  ...
  const corsiQuery = query(corsiCollection, where("facoltas", "array-contains", facoltaNome));
...
export async function getCorsoById(corsoId) {
  const courseTypes = ['triennale', 'phd', 'master-i', 'master-ii', 'master-ex', 'magistrale',"perfezionamento", 'academy'];
  ...
  for (const tipo of courseTypes) {
    ...
    try {
      ...
      console.error(`Errore durante il recupero del corso da ${tipo}: ${error.message}`);
  ...
  throw new Error("Corso non esistente in nessuna delle collezioni del database.");
}
